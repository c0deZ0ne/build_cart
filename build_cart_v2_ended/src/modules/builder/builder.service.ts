...
import { RemitterPaymentService } from '../payment/remiter-payment/remitter-payment.service';
import {
  PaymentType,
...
export class BuilderService {
  constructor(
    ...
    private remittancePaymentService: RemitterPaymentService,
  ...
  //   user,
  // }: {
  //   data: RemitterVerifyPaymentRefDto;
  //   user: User;
  // }) {
  //   const contractData = await this.contractService.getContractById(
  //     data.ContractId,
  //   );
  //   const vendorUserDetails = await this.userModel.findOne({where:{Vendor:contractData.VendorId},include:[{model:UserWallet}]})
  //   if (contractData.paymentStatus != ContractPaymentStatus.PENDING)
  //     throw new BadRequestException(
  //       'Payment already Payed or undergoing  verification',
  //     );
  ...
  //   try {
  //     const res = await this.remittancePaymentService.verifyPayment(
  //       data,
  //       user,
  //     );
  //     if (res) {
  ...
  //         contractId: data.ContractId,
  //         user,
  //         vend_token: data.pay_ref,
  //         dbTransaction: transaction,
  //       });
  //     await this.userTransactionService.createUserTransaction({
  //       UserWalletId:user.id,
  //       amount:Number(-data.pay_amount_collected),
  //       itemName:contractData.RfqQuote.RfqRequestMaterial.name,
  //       paymentMethod:PaymentMethod.BANK_TRANSFER,
  //       paymentProvider:PaymentProvider.PAYPAL,
  //       description:"Payment with remitter",
  //       reference:data.pay_ref,
  //       type:TransactionType.TRANSFER,
  //       CreatedById:user.id,
  //       status:TransactionStatus.COMPLETED,
  //       paymentType:"REMITTER",
  //       ProjectId:contractData.ProjectId,
  //       RfqRequestId:contractData.RfqRequestId,
  //       ItemTypes:ItemTypes.RFQ_REQUEST

  //     }, transaction);
  //     await this.userTransactionService.createUserTransaction({
  //       UserWalletId:vendorUserDetails.id,
  //       amount:Number(data.pay_amount_collected),
  //       itemName:contractData.RfqQuote.RfqRequestMaterial.name,
  //       paymentMethod:PaymentMethod.BANK_TRANSFER,
  //       paymentProvider:PaymentProvider.PAYPAL,
  ...
  //     vendorUserDetails.wallet.totalCredit+= data.pay_amount_collected
  //     await vendorUserDetails.save({transaction})
  //       await transaction.commit();
  //       return payment;
  //     }
  ...
  async remitaWalletTopUp({ user, pay_ref }: { user: User; pay_ref: string }) {
    return await this.remittancePaymentService.WalletTopUp({ user, pay_ref });
  }
  async paystackWalletTopUp({ user, pay_ref }: { user: User; pay_ref: string }) {
    return await this.paystackPaymentService.WalletTopUp({ user, pay_ref });
  }
  ...
  async payOrderWithRemita({
    ...
    return await this.remittancePaymentService.PayOrderWithRemita({
      orderId,
      user,
      pay_ref,
      description,
