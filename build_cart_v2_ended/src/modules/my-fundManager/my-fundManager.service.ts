...
export class MyFundManagerService {
  ...
  // /**
  //  * Retrieves all FundManager that belong to the current builder's list of my FundManager.
  //  * @param  User the logged in user that is a fundManager of the current  FundManagerId,
  //  * @returns list of FundManager Customers statistics.
  //  */
  // async getSponsorCustomers(user: User): Promise<any> {
  //   const customers = await this.mySponsorModel.findAll({
  //     where: {
  ...
  //       {
  //         model: Builder,
  //         attributes: ['name', 'logo', 'id', 'email', 'address'],
  //       },
  //       { model: Project },
  //     ],
  //   });
  //   if (!customers) return [];
  //   const cusData: SponsorCustomers[] = [];
  //   customers.map((myCus: MyFundManager) => {
  //     const currentCust = myCus.BuilderId;
  //     const checkCustomerExist: SponsorCustomers = cusData.find(
  //       (data) => data.id === currentCust,
  //     );
  //     if (!checkCustomerExist) {
  //       const customer: SponsorCustomers | null = {
  //         name: myCus.Builder.,
  //         logo: myCus.Builder.logo,
  //         id: myCus.BuilderId,
  //         projects: [myCus.Project],
  //         totalCredited: myCus.totalCredited,
  //         totalSpent: myCus.totalSpent,
  //         balance: myCus.balance,
  //         createdAt: myCus.createdAt,
  //         address: myCus.Builder.businessAddress,
  //       };

  //       cusData.push(customer);
  //     } else {
  //       checkCustomerExist.projects.push(myCus.Project);
  //       checkCustomerExist.totalCredited =
  //         Number(checkCustomerExist.totalCredited) +
  //         Number(myCus.totalCredited);

  //       checkCustomerExist.totalSpent =
  //         Number(checkCustomerExist.totalSpent) + Number(myCus.totalSpent);

  //       checkCustomerExist.balance =
  //         Number(checkCustomerExist.balance) + Number(myCus.balance);
  //     }
  //   });
