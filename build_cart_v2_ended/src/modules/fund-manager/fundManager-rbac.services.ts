...
@Injectable()
export class SponsorRolesService {
  constructor(
    @InjectModel(Role)
    ...
    private rolePermissionsModel: typeof RolePermission,
  ...
  async rolePermision({
    ...
    return await this.rolePermissionsModel.create({
      ...body,
      createdAt: new Date(),
      createdById: user.id,
    });
  }
  async removeRolePermision({
    user,
    body,
  }: {
    ...
    return await this.rolePermissionsModel.destroy({
      where: {
  ...
  async getAllRolesPermisionss({
    ...
    return await this.rolePermissionsModel.findAll({
  ...
  async createResourcesAccess({
    ...
    if (checkIfResourNameAlreadyExist)
      throw new BadRequestException(
        'This resource userType is already created and reference already exist share with users',
      );
