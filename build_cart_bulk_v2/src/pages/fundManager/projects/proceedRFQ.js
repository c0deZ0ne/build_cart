...
const ProceedRFQ = () => {
  ...
  const getRequestData = async (updateStage) => {
    try {
      let { data } = (await instance.get(`fundManager/rfq/${requestId}/bids`))
        .data;
      ...
      if (data?.orders[0]?.status === "PAID") {
        stage = "DELIVERYSCHEDULE";
      }

      if (data?.orders[0]?.pendingCount === 0) {
              ...
              >
                ...
                <Flex>
                  ...
                  <Text color="#1C903D" as="span">
                    {new Intl.NumberFormat().format(request?.totalBudget ?? 0)}
                  </Text>
                </Flex>
              </Flex>
            </Flex>
