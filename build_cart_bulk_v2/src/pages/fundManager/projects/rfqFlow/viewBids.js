...
const ViewBids = ({ data, getRequestData }) => {
  ...
  useEffect(() => {
    if (bidItem?.RfqRequestMaterial) {
      const metric = measurementOptions.find(
        (e) => e?.value === bidItem?.RfqRequestMaterial?.metric
      );

      setValue("itemName", bidItem?.RfqRequestMaterial?.name);
      setValue("quantity", bidItem?.RfqRequestMaterial?.quantity);
      setValue(
        "deliveryDate",
        moment(bidItem?.deliveryDate).format("YYYY-MM-DD")
      );
      setValue("measurement", metric ?? measurementOptions[0]);
      setValue("price", bidItem?.RfqRequestMaterial?.budget);
  ...
  const handleAcceptBid = async (quoteId) => {
    ...
    try {
      ...
      setLoadingBtn(false);
      handleSuccess("Bid has been successfully accepted");
  ...
  const onSubmit = async (fields) => {
    try {
      const payload = {
        ProjectId: bidItem?.RfqRequestMaterial?.ProjectId,
        RfqQuoteId: bidItem?.id,
        price: Number(fields?.price),
        description:
          fields?.additionalInformation ??
          bidItem?.RfqRequestMaterial?.description,
      };
  ...
  return (
    <Box my="40px" p={"32px 24px"} background={"#FCF7F6"}>
      ...
      <BaseModal
        ...
        size="xl"
        title={"Bargain"}
        subtitle={"Input how long it will take to deliver items"}
        reset={reset}
      >
