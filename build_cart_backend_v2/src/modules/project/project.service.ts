...
export class ProjectService {
  ...
  async createdProject({
    ...
    const emailsToInvite = undefined;
    ...
    try {
      ...
      try {
        extractEmailAndUUIDFundmanagerData.emails?.forEach(async (d) => {
          const [newInvite] = await this.invitationModel.findOrCreate({
            defaults: {
              projectId: newProject.id,
              buyerEmail: d || '',
  ...
  //create the pending transaction here
      ...
      await this.projectTransactionModel.create(
        {
          ...
          amount,
          ProjectId: project.id,
          description: SystemPaymentPurpose.FUND_PROJECT_WALLET
