...
export default function BuilderFundManagerProject() {
  ...
  useEffect(() => {
    if (projects) {
      const adjustedProjects = projects?.data?.projects.map(
        (project, index) => {
          return {
            ...
            duration:
              weeksDiff(project?.startDate, project?.endDate) + " weeks",
            bids: "No data",
            status: <Badge status={project?.status} />,
            dateCreated: moment(project?.createdAt).format("DD-MM-YYYY"),
            action: (
              <Link to={`/builder/company-project/details/${project?.id}`}>
      ...
      setAllProjects(projectStat?.data?.allProjects ?? 0);
      setActiveProjects(projectStat?.data?.activeProjects ?? 0);
      setPendingProjects(projectStat?.data?.pendingProjects ?? 0);
      setCompletedProjects(projectStat?.data?.completedProjects ?? 0);
      setTransactions(projectStat?.data?.transactionResolutionProjects ?? 0);
    }
    ...
    <DashboardWrapper pageTitle="Project">
      ...
      >
        <Box mb={{ base: "10px", md: "0px" }}>
          ...
          <Text fontSize="14px">
            Your company manages these projects, but they are financed by a
            third-party institution or project financier.
          </Text>
        </Box>
        <Box>
          <Button
      ...
      {searchTable().length === 0 && !isLoading ? (
        ...
        >
          <EmptyState>
            <Text>
              Your company currently has no projects for{" "}
              <Text as="span" color="#12355A">
                third-party institutions or fund managers.
              </Text>{" "}
              When projects for third-party institutions or fund managers are
              created, they will be displayed here.
            </Text>
          </EmptyState>
        </Box>
      ) : (
          ...
          <Flex justifyContent="space-between" align="center" mb="24px">
            <Box>
              ...
              <Text color="primary" fontSize="14px">
                Content for fund manager projects
              </Text>
            </Box>
