...
import { useCreateBuilderProjectMutation } from "../../../../redux/api/super-admin/builderSlice";

export default function CreateProjectBuilder({
  refetch,
  isOpen,
  onClose,
  project = null,
  ...
  const submitProject = async (data) => {
    if (project?.id) {
      ...
      createBuilderProject({
        id: builderId,
        payload: {
          location: data.projectLocation.value,
          startDate: new Date(data.startDate),
          endDate: new Date(data.endDate),
          title: data.projectTile,
          ...
          image: img && img.url,
          fundManagerId: data.selectFundManager?.id,
  ...
  const getFundManagers = async () => {
    ...
    try {
      const { data } = (
        await instance.get(`/superAdmin/fundManagers?page_size=5000`)
      ).data;
