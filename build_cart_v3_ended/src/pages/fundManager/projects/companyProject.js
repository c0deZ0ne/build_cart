...
export default function FundManagerCompanyProject() {
  ...
  const [activeCard, setActiveCard] = useState("All Projects");
    ...
    if (projects) {
      const adjustedProjects = projects?.data.map((project, index) => {
        return {
          ...
          duration: weeksDiff(project?.startDate, project?.endDate) + " weeks",
          bids: "No data",
          status: <Badge status={project?.status} />,
          dateCreated: moment(project?.createdAt).format("DD-MM-YYYY"),
          action: (
            <Link to={`/fund-manager/project/details/${project?.id}`}>
      ...
      setTransactions(projectStat?.data?.transactionsResolutions ?? 0);
    }
